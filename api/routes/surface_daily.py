...
surfaced_picks row. Selection rules (from phase-4-rev-plan.md §Step 8):
  ...
  2. Pick at most 3, trying to vary kind.
  3. Fewer than 3 available → surface what exists (length ≤ 3 accepted per F8).
  4. Mark selected items state='surfaced'. Write surfaced_picks.
"""
...
def surface_daily(
    ...
    pending: list[dict] = rows_resp.data or []
    ...
    # 2. Pick up to 3 with kind variety.
    picked = _pick_varied(pending)

    # 3. Mark picked items as 'surfaced'.
