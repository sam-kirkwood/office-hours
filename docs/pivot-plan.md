...
Next step: Phase 10-rev Step 4 — queue UX polish. Decide D3 (concept_review content source — default lean: reuse problem pool by subtopic tag, fall back to node description + subtopics) before starting. Then implement concept_review item rendering (#3), paper request flow auto-falling back to /propose-papers when /suggest-papers returns empty (#5), and non-blocking queue requests with fire-and-forget UX (#39).
