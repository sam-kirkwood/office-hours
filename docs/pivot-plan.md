...
**Where things are (2026-06-22):** Phase 12 (the responsive daily loop) is complete.
...
generation, subtopic drill replaced); Step 4a done (orientation orchestrator +
A/B/C/D signal state + resumability + form-mode gaps; new `orientation_sessions`
table). Step 4b (the system prompt) is next — its own session.
...
| **13** | conversational orientation — four-signal tutor, rich paths, the entry-point amendment | 🔄 Steps 1–3 + 4a done |
...
**Pending operator action** (Phase 13 Step 4a): apply migration `20250040`
(`orientation_sessions` table) before `POST /orientation-tutor` runs against the
live DB — tests use FakeSupabase so they pass without it.
