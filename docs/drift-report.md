...
| 18 | API Routes | `suggest_papers.py` pre-filters on `interest_titles[0]` only instead of `ILIKE ANY(ARRAY[...])` across all titles | `diverged` | `todo` | Conflicts with docs/pivot-plan.md §F16 — full ILIKE ANY across all interest titles, capped at 20 rows |
| 19 | API Routes | `update_queue.py` recomputes node state only on `attempt_submit`; completing a paper engagement does not update `user_node_states` | `missing` | `todo` | Conflicts with ARCHITECTURE.md §Engaging with a paper step 5; SPEC.md §Adaptation |
| 20 | API Routes | `compute_cross_pollination.py` cooldown uses `added_at >= cutoff` only; a dismissed suggestion doesn't block re-suggestion of a different node in the same week | `diverged` | `todo` | Conflicts with docs/graph-design.md §Cross-pollination — "hold off on similar suggestions for a while." |
| 21 | API Routes | `add_interest.py` handles `same`/`related`/`new` only; `split` and `vague` verdicts fall through silently to the `new` branch | `diverged` | `todo` | Conflicts with docs/graph-design.md §Deduplication; ARCHITECTURE.md §Adding an interest |
| 22 | API Routes | `ingest_paper_user.py` does not deduplicate on `(user_id, paper_id)` before generating an engagement; re-submitting the same URL creates a duplicate | `diverged` | `todo` | Conflicts with ARCHITECTURE.md §Generating a paper engagement — "dedupe by arxiv_id / doi / title." |
| 23 | API Routes | `suggest_papers.py` and `propose_papers.py` can both insert duplicate queue items for the same engagement if called concurrently | `diverged` | `todo` | Conflicts with ARCHITECTURE.md §Queue — single-item-per-engagement-per-user; docs/pivot-plan.md §update_queue dedup step |
